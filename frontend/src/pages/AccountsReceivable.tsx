...
import { AccountReceivable, Customer, PaginatedResponse } from '../types';
import { toast } from 'sonner';
import { Plus, Search, Trash2, DollarSign, X, Check } from 'lucide-react';
...
const AccountsReceivable = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['accounts-receivable', page, search, statusFilter],
    queryFn: async () => {
      const response = await api.get<PaginatedResponse<AccountReceivable> & { summary: any }>('/accounts-receivable', {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => api.post('/accounts-receivable', data),
    onSuccess: () => {
  ...
  const markAsPaidMutation = useMutation({
    mutationFn: (id: string) => api.put(`/accounts-receivable/${id}/mark-paid`, {}),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/accounts-receivable/${id}`),
    onSuccess: () => {
  ...
  const handleMarkAsPaid = (id: string) => {
    if (window.confirm('Confirmar recebimento desta conta?')) {
      ...
      {data?.summary && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="card bg-green-50 border-green-200">
            <p className="text-sm text-green-600 mb-1">Recebido</p>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Nenhuma conta a receber encontrada</p>
          </div>
        ) : (
          <>
